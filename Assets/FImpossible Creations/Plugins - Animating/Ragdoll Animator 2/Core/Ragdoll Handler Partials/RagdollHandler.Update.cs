...
namespace FIMSpace.FProceduralAnimation
{
    public partial class RagdollHandler
    {
        ...
        private float GetBoneSpringValue(RagdollChainBone bone, float power, float spring) => power * spring * bone.ParentChain.MusclesForce * bone.ForceMultiplier + bone.MusclesBoost * spring * targetMusclesPower;
        private float GetBoneDampValue(float damping) => damping * targetMusclesPower;

        public void User_UpdateBoneJointPlayParameters(RagdollChainBone bone)
        {
            float damping;
            float spring;
            ...
            bone.SetJointMatchingParameters(GetBoneSpringValue(bone, power, spring), GetBoneDampValue(damping));

            if (WasInitialized) bone.GameRigidbody.ResetInertiaTensor();
        }

...
public class StylizedWaterVRGUI : ShaderGUI
{
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        {
            // Toggle for _REFRACTION keyword
            MaterialProperty refractionToggle = FindProperty("_UseRefraction", properties, false);
            if (refractionToggle != null)
            {
                EditorGUI.BeginChangeCheck();
                bool enabled = refractionToggle.floatValue > 0.5f;
                enabled = EditorGUILayout.Toggle("Enable Refraction", enabled);
                if (EditorGUI.EndChangeCheck())
                {
                    refractionToggle.floatValue = enabled ? 1f : 0f;
                    if (enabled)
                        mat.EnableKeyword("_REFRACTION");
                    else
                ...
                if (!enabled)
                {
                    DrawHelpBox("Refraction OFF — saves 1 fullscreen texture read per pixel");
                }
            }
    ...
    // ════════════════════════════════════════════════════════════════
    // Drawing Helpers (same pattern as other GUIs)
    // ════════════════════════════════════════════════════════════════
